      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, size: 20, color: colors.inkPrimary),
          ),
    ...
    return GridView.count(
      ...
      children: [
        for (final item in items)
          _ActionTile(icon: item.icon, label: item.label, onTap: item.onTap),
      ],
    );
  }
}
...
/// Small gold-tinted points chip — watches [appRewardsProvider], tap
/// navigates to the Rewards shell tab. Mirrors the tint-chip pattern used by
/// `_LabelChip` in `np_balance_card.dart`.
///
/// NOTE: design-tokens.md scopes `gold.reward` to the rewards screens
/// (I1/I2) only. This chip is an explicit spec requirement for B1 (Home
/// Dashboard §7) and is a deliberate, narrow exception to that rule — flag
/// for confirmation alongside the rest of this deliverable.
class _RewardsChip extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return rewardsAsync.when(
      data: (rewards) => GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.star_rounded, size: 16, color: colors.accentDeep),
              const SizedBox(width: AppSpacing.xs),
              Text(
                '${rewards.points} ${l10n.homeRewardsPointsSuffix}',
      ...
      loading: () =>
          const NPShimmer(width: 96, height: 32, borderRadius: AppRadius.chip),
      error: (error, stackTrace) => GestureDetector(
        ...
        child: Container(
          ...
          child: Text(l10n.homeErrorRetry, style: textTheme.labelLarge),
        ),
      ),
    );
  }
